...
define("database", "mysql"); // your database 
...
class Private_messaging_system{
	...
	**/
	...
	public function get_all_messages(){
		$role = "sender_delete";
		$id = $_SESSION['user_id'];
		$query = mysqli_query("SELECT user_to FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "'");
	...
	public function get_message($message_id){
		...
		$query = mysqli_query("SELECT * FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "' AND (user_to = '" . $id . "' OR user_from = '" . $id . "') OR respond = '" . $message_id . "' AND " . $role . " != 'n'");
		return $query;
	} // END get_message
	...
	public function delete_message($message_id){
		$role = "sender_delete";
		$id = $_SESSION['user_id'];
		$query = mysqli_query("SELECT user_to FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "' OR respond = '" . $message_id . "'");
		while($data = mysqli_fetch_object($query)){
			if($data->user_to != $id){
				$role = "receiver_delete";
			}			
		...
		$query1 = mysqli_query("UPDATE " . TBL_MESSAGES . " SET " . $role . " = 'y' WHERE id = '" . $message_id . "'");
	...
	public function delete_conversation($conversation_id){
		...
		mysqli_query("UPDATE " . TBL_MESSAGES . " SET " . $role . " = 'y' WHERE id = '" . $conversation_id . "'");
	}

	private function _check_for_deleted_messages(){
		mysqli_query("DELETE FROM " . TBL_MESSAGES . " WHERE sender_delete = 'y' AND receiver_delete 'y'"); // removes messages from DB if both sender and receiver have deleted it.
	...
	**/
	private function _email_user_of_new_message($to,$from,$subject){
		$r = mysqli_fetch_object(mysqli_query("SELECT first_name,last_name,email FROM " . TBL_USERS . " WHERE id = '" . $to . "'"));
		$u = mysqli_fetch_object(mysqli_query("SELECT first_name,last_name FROM " . TBL_USERS . " WHERE id = '" . $from . "'"));
