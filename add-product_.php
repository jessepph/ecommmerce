...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['vendor_name'])) {
        $vendorNameInput = mysqli_real_escape_string($con, $_POST['vendor_name']);

        // Check if vendor_name input matches logged-in username
        if ($vendorNameInput !== $username) {
            // Output CSS for centered notification box
echo '<style>';
echo '.alertify-notifier {';
echo '  width: 400px !important;'; // Adjust width as needed
echo '  height: auto !important;'; // Adjust height as needed
echo '  font-size: 16px !important;'; // Adjust font size as needed
echo '  padding: 20px !important;'; // Adjust padding to make the notification box larger
echo '  position: fixed !important;'; // Ensure the position is fixed
echo '  top: 50% !important;'; // Center vertically
echo '  left: 70% !important;'; // Center horizontally
echo '  transform: translate(-50%, -50%) !important;'; // Center horizontally and vertically
echo '  z-index: 9999 !important;'; // Ensure it's above other elements
echo '}';
echo '</style>';

// Output JavaScript to show Alertify error message and prevent form submission
echo '<script>';
echo '$(document).ready(function () {';
echo 'alertify.set("notifier", "position", "top-right");'; // Set position (position will be overridden by CSS)
echo 'alertify.set("notifier", "delay", 20);'; // Set duration in seconds
echo 'alertify.error("You must type in the exact name of your vendor account. Simply match the text within the field; all other entries are not permitted. To retry adding your product please select add product from the cpanel and start over.");';
echo '});';
echo '</script>';
        } else {
            ...
            $terms_refund_policy = mysqli_real_escape_string($con, $_POST['terms_refund_policy']);
            ...
            $ships_from = mysqli_real_escape_string($con, $_POST['ships_from']);
            $ships_to = mysqli_real_escape_string($con, $_POST['ships_to']);
            $filename = '';
            ...
            $product_query = "INSERT INTO products (category_id, name, slug, product_description, terms_refund_policy, original_price, selling_price, qty, vendor_name, meta_title, meta_keywords, status, trending, image, ships_from, ships_to) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            $stmt = mysqli_stmt_init($con);
            if (mysqli_stmt_prepare($stmt, $product_query)) {
                mysqli_stmt_bind_param($stmt, "isssssddssssssss", $category_id, $name, $slug, $product_description, $terms_refund_policy, $original_price, $selling_price, $qty, $vendorNameInput, $meta_title, $meta_keywords, $status, $trending, $filename, $ships_from, $ships_to);
                ...
                if (mysqli_stmt_affected_rows($stmt) > 0) {
                    ...
                    echo 'alertify.success("Product Added Successfully ");';
                    echo '});';
                    echo '</script>';
                    ...
                    echo '$(document).ready(function () {';
                    echo 'alertify.set("notifier", "position", "top-right");';
                    echo 'alertify.error("Something went wrong");';
                    echo '});';
                    echo '</script>';
                ...
                echo '$(document).ready(function () {';
                echo 'alertify.set("notifier", "position", "top-right");';
                echo 'alertify.error("Failed to prepare statement");';
                echo '});';
                echo '</script>';
