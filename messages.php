...
$message_query = "SELECT id, FromUser, receive_date, subject, message FROM messages WHERE ToUser = ?"; // Adjust column name
$stmt = $conn->prepare($message_query);
if ($stmt) {
    $stmt->bind_param("s", $username); // Assuming messages are fetched for the logged-in user
    $stmt->execute();
...
$badge_class2 = $unread_count > 0 ? 'badge-danger' : 'badge-grey';
...
// Initialize variables for badge
$badge_color2 = 'grey'; // Default color
$badge_text2 = '0';     // Default text

if ($result) {
    $row = $result->fetch_assoc();
    $unread_count = $row['unread_count'];

    // Set badge properties based on the unread count
    if ($unread_count > 0) {
        $badge_color2 = 'red';   // Set badge color to red if there are unread messages
        $badge_text2 = $unread_count; // Display the number of unread messages
    }
} else {
    echo "Error fetching unread messages count: " . $conn->error;
}
...
?>
    ...
    <span class="badge" style="
        ...
        background-color: <?php echo htmlspecialchars($badge_color2); ?>;
        color: white;
        ">
...
// After calculating the total item count
$totalItemCount = 0; // Initialize total item count
...
// Determine badge styles and content for the cart
if ($totalItemCount > 0) {
    $badge_class = 'badge-danger'; // Class for red badge
    $badge_color = 'red';          // Background color
    $badge_text = $totalItemCount; // Display the total item count
} else {
    $badge_class = '';             // No class for grey badge
    $badge_color = 'grey';         // Background color for zero items
    $badge_text = '0';             // Show '0' when there are no items
}
...
if ($result) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // No rows found, set default values
        $unread_count = 0;
        $badge_color = 'grey';
        $badge_text = '0';
        $badge_class = '';
...
if ($result) {
    ...
    if ($unread_count > 0) {
        ...
        $badge_color2 = 'grey';  // Set badge color to grey if no unread messages
        $badge_class2 = '';      // Remove badge class
    }
} else {
    // Handle query error
    echo "Error fetching unread messages count: " . $conn->error;
...
// Query to count items in the cart for the current user
$sql = "SELECT SUM(quantity) AS total_quantity FROM cart WHERE username = '$safe_current_user'";
$result = $conn->query($sql);

// Initialize variables for badge
$badge_color = 'grey'; // Default color for no items
$badge_class = '';     // Default class for no items
$badge_text = '0';     // Default text

if ($result) {
    $row = $result->fetch_assoc();
    $totalItemCount = intval($row['total_quantity']); // Get total quantity

    if ($totalItemCount > 0) {
        $badge_class = 'badge-danger'; // Class for red badge
        $badge_color = 'red';          // Background color for red
        $badge_text = $totalItemCount; // Display the total item count
    } else {
        // No need to set badge_class or badge_color, defaults will apply
        $badge_text = '0'; // Explicitly show '0' when there are no items
    }
} else {
    echo "Error fetching cart item count: " . $conn->error;
}
...
<li class="right shopping-cart-link">
    <a href="messages.php">
        ...
        &nbsp;<span class="badge <?php echo $badge_class2; ?>" style="
        padding: 0.3em 0.4em;
        font-size: 75%;
        font-weight: 700;
        top: 24px;
        ...
        color: white;
        border-radius: 0.25rem;
        background-color: <?php echo $badge_color2; ?>;">
        <?php echo htmlspecialchars($badge_text2); ?>
        </span>
    </a>
</li>
...
<div class="wrapper">
    <div class="row">
        <div class="col-md-3 sidebar-navigation">
            <!-- Sidebar navigation code remains unchanged -->
        </div>
        <div class="col-md-9 sidebar-content-right">
            <div class="container nopadding" style="margin-left:-270px;">
...
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
                                    ...
                                    $message_query = "SELECT id, FromUser, receive_date, subject, message FROM messages WHERE ToUser = ?";
$stmt = $conn->prepare($message_query);
if ($stmt) {
    $stmt->bind_param("s", $username); // Assuming messages are fetched for the logged-in user
    $stmt->execute();
    ...
    die("Database query preparation failed: " . $conn->error);
                                    ...
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        <tr>
                                            <td><input type="checkbox" name="message_ids[]" value="<?php echo $row['id']; ?>" class="message-checkbox"></td>
                                            <td><?php echo htmlspecialchars($row['FromUser']); ?></td>
                                            <td><?php echo date('d-m-Y H:i:A', strtotime($row['receive_date'])); ?></td>
                                            <td><?php echo htmlspecialchars($row['subject']); ?></td>
                                            <td><?php echo htmlspecialchars($row['message']); ?></td>
                                            <td class="action-buttons">
                                                <a href="view-message.php?id=<?php echo $row['id']; ?>">View</a>
                                                <a href="reply-message.php?to=<?php echo urlencode($row['FromUser']); ?>&subject=<?php echo urlencode($row['subject']); ?>">Reply</a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
